      ...
      {checkList.map((item, index) => (
        ...
        >
          ...
          <img
            src="/Icon.svg"
            alt="checkmark"
            className=" flex-shrink-0 relative mr-1 top-1 h-4 w-4 lg:top-0.75 lg:h-5 lg:w-5 "
          />
          <span className="leading-6 text-t-custom-black flex-1">{item}</span>
        </div>
      ))}
      <span className=" relative text-t-gray space-x-1 text-sm top-2 lg:text-base">
        Have questions? Visit our
        <a className="text-t-purple font-bold"> Help Center </a> or reach out to
        our
