...
const Page = ({ phone ,mode,onGoBack}: { phone: string ;mode:'signup'|'login';onGoBack:() => void;}) => {
  ...
  return (
    <div className="flex justify-center items-center p-10 flex-col lg:p-24">
      ...
      <div className="flex flex-col items-center mt-10 lg:mt-15">
        <span className="text-t-gray text-2xl font-semibold whitespace-nowrap">Enter OTP</span>
        <span className="text-center whitespace-nowrap text-t-second-gray mt-2 text-base font-normal px-1 lg:px-0">
      ...
      <div className="flex flex-col items-center">
        ...
        <div className=" w-full max-w-full px-0 mt-1  relative lg:px-0">
          <button className={`relative w-full flex-grow bg-btn-bg text-white rounded-md py-3 mt-5`}
            onClick={handleVerify}>Verify </button>

        </div>
      </div>
      <div className='mt-7 text-t-gray font-normal text-md'>Didn’t receive code? <Link href='/tep' className='text-t-purple font-semibold text-md ml-1'> Resend OTP</Link></div>  
      <button
                onClick={onGoBack}
